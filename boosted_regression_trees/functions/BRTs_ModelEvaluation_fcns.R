...
eval_7525_heather_fixed_50_reviewer4 <- function(dataInput, gbm.x, gbm.y, lr, tc=tc,tolerance.method,tolerance,family,response,eval_name,variable){
  ...
  for(i in 1:50){
    ...
    test=dataInput %>% group_by(sum_gaps) %>% summarise(n=n())
    
    A0=dataInput %>% filter(sum_gaps==0)%>% .[sample(nrow(.),min(test$n)),]
    A1=dataInput %>% filter(sum_gaps==1)%>% .[sample(nrow(.),min(test$n)),]
    
    master=do.call("rbind",list(A0,A1))
    test=master %>% group_by(sum_gaps) %>% summarise(n=n())
    DataInput <- master
