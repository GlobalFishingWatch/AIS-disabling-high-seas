...
#### 5111A #####
## 15 Fishing: pres/abs; abs vessel presence, everything is 50nm from shore MPA instead of NTA ####
date="06_17_22c"
type="PresAbs"
dist="50nm"
      ...
      expr ={
        if(vessels[i]!="all"){
          main=read.csv(glue("/Users/heatherwelch/Dropbox/IUU_GRW/fishing_activity_modeling_2017_2019/extracto/fishing_classAB_sat_200nm_{date_fish}_{vessels[i]}_VC.csv"))%>% 
            dplyr::select(c(unique,loitering_hours,dist_mpa,dist_gfw,l.chl,sst,sst_temporal_sd,l.eke,sum_gaps,dist_ASAM_events_allyears_2019))
        }
        if(vessels[i]=="all"){
        ...
        main=main %>% mutate(random=sample(1:nrow(main))) %>%
          ...
          mutate(dist_ASAM_events_allyears_2019=replace(dist_ASAM_events_allyears_2019,dist_ASAM_events_allyears_2019>400000,400000))  
        main=main %>% mutate(sum_gaps=as.integer(round(sum_gaps))) %>% .[complete.cases(.),]
        
        #full model no reception
        gbm.x=c("loitering_hours","dist_gfw","l.chl","sst","sst_temporal_sd","l.eke","random","dist_mpa","dist_ASAM_events_allyears_2019")
        family="bernoulli"
        ...
        tryCatch(
          expr ={
            ...
            name=glue("/Users/heatherwelch/Dropbox/IUU_GRW/SDMs/brt_06.06.22/fishing_classAB_sat_200nm_12hr_{date_fish}_{distance_vars}_step_lr{lr}_tc{tc}_bf{bf}_tol{tolerance}_{family}_{type}.rds")
...
## 16 vessel presence: pres/abs; abs background, everything is 50nm from shore MPA instead of NTA ####
date="06_17_22c"
type="PresAbs"
dist="50nm"
abs_f="background"
      ...
      expr ={
        ...
        tryCatch(
          expr ={
            ...
            name=glue("/Users/heatherwelch/Dropbox/IUU_GRW/SDMs/brt_06.06.22/vespres_classAB_sat_200nm_12hr_{date_fish}_{distance_vars}_step_lr{lr}_tc{tc}_bf{bf}_tol{tolerance}_{family}_{type}.rds")
            write_rds(gap_brt_poiss_step,name)
          },
          error = function(e){
            message(glue("Model not working"))
      ...
      expr ={
        ...
        tryCatch(
          expr ={
            ...
            name=glue("/Users/heatherwelch/Dropbox/IUU_GRW/SDMs/brt_06.06.22/gaps_classAB_sat_200nm_12hr_{date2}_{distance_vars}_step_lr{lr}_tc{tc}_bf{bf}_tol{tolerance}_{family}_{type}.rds")
            write_rds(gap_brt_poiss_step,name)
          },
          error = function(e){
            message(glue("Model not working"))
